...
class ObjectCard(QWidget):
    ...
    def _setup_ui(self) -> None:
        ...
        self.station_combo.addItem("Station 1")
        self.station_combo.addItem("Station 2")
        self.station_combo.setToolTip(
            "Bind this USB/OSC object to a Croissant LilyGO. "
            "Play/Stop and story gating go only to that board."
        )
    ...
    def _on_station_changed(self, _index: int) -> None:
        n = self.station_combo.currentIndex()
        if self._communication_type == "OSC" and n in (1, 2) and hasattr(self, "address_edit"):
            cur = self.address_edit.text().strip()
            if cur.startswith("/red_dust/croissant"):
                self.address_edit.blockSignals(True)
                self.address_edit.setText(f"/red_dust/croissant/{n}")
    ...
    def get_config(self) -> dict:
        ...
        station = self.station_combo.currentIndex()
        if station in (1, 2):
            config["croissant_station"] = station
    ...
    def set_config(self, config: dict) -> None:
        ...
        raw_station = config.get("croissant_station")
        try:
            station_idx = int(raw_station) if raw_station is not None else 0
        except (TypeError, ValueError):
            station_idx = 0
        if station_idx not in (1, 2):
            station_idx = 0
...
class ObjectCardsContainer(QFrame):
    ...
    def _add_object(
        ...
        if emit_added:
            assigned = {
                c.get_config().get("croissant_station")
                for c in self._cards.values()
                if c is not card
            }
            for n in (1, 2):
                if n not in assigned:
                    card.station_combo.blockSignals(True)
                    card.station_combo.setCurrentIndex(n)
                    card.station_combo.blockSignals(False)
                    if communication_type == "OSC" and hasattr(card, "address_edit"):
                        card.address_edit.blockSignals(True)
                        card.address_edit.setText(f"/red_dust/croissant/{n}")
                        card.address_edit.blockSignals(False)
                    break
