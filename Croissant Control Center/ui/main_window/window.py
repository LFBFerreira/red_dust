"""
Main Window for Croissant Control Center.
"""
...
class MainWindow(
    ...
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Croissant Control Center")
        ...
        self.story_clocks = [StoryClock() for _ in range(STATION_COUNT)]
        self.pin_cues_list = [PinCueList(pin_count=STORY_PIN_COUNT) for _ in range(STATION_COUNT)]
        self.speed_envelopes = [SpeedEnvelope() for _ in range(STATION_COUNT)]
        self.osc_manager.set_story_sources(self.story_clocks, self.pin_cues_list)
        ...
        logger.info("Croissant Control Center initialized")
    ...
    def showEvent(self, event: QShowEvent) -> None:
        ...
        if not self._splits_initialized:
            ...
            QTimer.singleShot(0, self._sync_layout_after_show)
    ...
    def closeEvent(self, event: QCloseEvent) -> None:
        ...
        try:
            ...
            if getattr(self, "story_clocks", None):
                for station, clock in enumerate(self.story_clocks):
                    timeline = (
                        self.story_timelines[station]
                        if getattr(self, "story_timelines", None)
                        and station < len(self.story_timelines)
                        else None
                    )
                    if (
                        timeline is not None
                        and timeline.sync_dy_enabled()
    ...
    def _setup_ui(self):
        ...
        Layout (horizontal splitter; Serial / OSC sit to the right, matching Dust Devil):

            ┌─ left ──────────────────────────────────┬─ objects ─┐
            │  Dataset Information │ Waveform Viewer  │ Serial /  │
            │  Data Picker         │ Playback         │ OSC cards │
            │  Station 1 / 2 timelines + Speed        │           │
            │  Log (optional)                         │           │
            └─────────────────────────────────────────┴───────────┘
        """
        ...
        # --- Left bottom: Station 1 / 2 timelines + speed envelopes + optional log ---
        self._bottom_splitter = QSplitter(Qt.Orientation.Vertical)
        self._bottom_splitter.setChildrenCollapsible(False)

        self.story_timelines = []
        stations_host = QWidget()
        stations_layout = QVBoxLayout(stations_host)
        stations_layout.setContentsMargins(0, 0, 0, 0)
        stations_layout.setSpacing(WIDGET_PANEL_MARGIN)
        for station in range(STATION_COUNT):
            panel = StoryTimelinePanel(
                self.pin_cues_list[station],
                self.story_clocks[station],
                self.speed_envelopes[station],
                title=STATION_TITLES[station],
                pin_labels=STATION_PIN_LABELS[station],
                lane_color_offset=station * STORY_PIN_COUNT,
            )
            self.story_timelines.append(panel)
            stations_layout.addWidget(panel, 1)
        self._story_timeline_panel = wrap_panel(stations_host)
    ...
    def _sync_layout_after_show(self) -> None:
        """Re-apply splits after the first real layout pass (showEvent widths can be too small)."""
        self._sync_main_column_split()
        self._sync_left_row_split()
        self._sync_top_column_split()
        self._remember_main_right_width()
    ...
    def _sync_top_column_split(self) -> None:
        ...
        if total < LEFT_PANEL_WIDTH + 200:
            return
        self._top_column_splitter.setSizes(
            [LEFT_PANEL_WIDTH, max(1, total - LEFT_PANEL_WIDTH)]
        )

    def _sync_left_row_split(self) -> None:
        """Initial 50/50 split between waveform and the station timelines on the left."""
    ...
    def _apply_log_visibility(self, visible: bool) -> None:
        """Show or hide the log panel; the timelines fill the left bottom when hidden."""
        if visible:
