...
from settings import (
    SERIAL_BAUDRATE,
    OSC_OUTPUT_INTERVAL_MS,
    SERIAL_OUTPUT_INTERVAL_MS,
    STATION_COUNT,
    STORY_PIN_COUNT,
)
...
class OSCManager(QObject):
    ...
    dy_button_received = Signal(int, str)
    ...
    def __init__(self, waveform_model=None, playback_controller=None):
        ...
        self._story_clocks: List[StoryClock] = []
        self._pin_cues_list: List[PinCueList] = []
    ...
    def set_story_sources(
        self,
        story_clocks: Optional[List[StoryClock]],
        pin_cues_list: Optional[List[PinCueList]],
    ) -> None:
        self._story_clocks = list(story_clocks or [])
        self._pin_cues_list = list(pin_cues_list or [])

    def apply_croissant_station(self, object_id: str, station) -> None:
        """Bind an object to station 1 or 2 (1-based). Other values clear the binding."""
        obj = self._objects.get(object_id)
        if obj is None:
            return
        try:
            n = int(station) if station is not None else 0
        except (TypeError, ValueError):
            n = 0
        if n in (1, 2):
            obj.croissant_station = n - 1
        else:
            obj.croissant_station = None

    def send_dy_command(self, play: bool, station: int = 0) -> int:
        """Send ``DY,PLAY`` / ``DY,STOP`` to that station's Serial object only (dust_devil contract)."""
        if station < 0 or station >= STATION_COUNT:
            logger.warning("Invalid Croissant DY station %s", station)
            return 0
        line = "DY,PLAY\n" if play else "DY,STOP\n"
        serial_objs = [obj for obj in self._objects.values() if isinstance(obj, SerialObject)]
        targets = [obj for obj in serial_objs if obj.croissant_station == station]
        if not targets:
            logger.warning(
                "No Serial object bound to Croissant station %d; not broadcasting DY",
                station + 1,
            )
            return 0
        sent = 0
        for obj in targets:
            if obj.send_control_line(line):
                sent += 1
                logger.info("Sent %s to %s (station %d)", line.strip(), obj.object_id, station + 1)
            else:
                logger.warning(
    ...
    def _poll_serial_input(self) -> None:
        for obj in self._objects.values():
            ...
            for line in obj.read_available_lines():
                self._handle_device_line(line, obj)

    def _handle_device_line(self, line: str, obj: Optional[InteractiveObject] = None) -> None:
        text = line.strip().upper().replace(" ", "")
        parts = text.split(",")
        cmd = None
        station = obj.croissant_station if obj is not None else None
        if len(parts) == 3 and parts[0] == "DY" and parts[1] == "BTN":
            cmd = parts[2]
        elif len(parts) == 4 and parts[0] == "DY" and parts[1] == "BTN":
            try:
                station = int(parts[2]) - 1
            except ValueError:
                return
            cmd = parts[3]
        if cmd is None:
            return
        if station is None or station < 0 or station >= STATION_COUNT:
            logger.warning("DY button %s from %s has no Croissant station binding", cmd, getattr(obj, "object_id", "?"))
            return
        if cmd in ("PLAY", "START"):
            logger.info("Croissant station %d device button: PLAY", station + 1)
            self.dy_button_received.emit(station, "PLAY")
        elif cmd == "STOP":
            logger.info("Croissant station %d device button: STOP", station + 1)
            self.dy_button_received.emit(station, "STOP")
    ...
    def _build_ordered_values(
        ...
        for s in range(width):
            ...
            if r is None:
                ...
                ordered.append((r.row_id, self._apply_story_gate(obj, s, n)))
        return ordered

    def _apply_story_gate(self, obj: InteractiveObject, slot_index: int, value: float) -> float:
        """Gate by the object's bound station, or by Pin_A/B vs Pin_C/D slots if unbound."""
        if slot_index < 0:
            return value
        if obj.croissant_station is not None:
            station = obj.croissant_station
            local_pin = slot_index % STORY_PIN_COUNT
        else:
            station = slot_index // STORY_PIN_COUNT
            local_pin = slot_index % STORY_PIN_COUNT
        if (
            station >= STATION_COUNT
            or station >= len(self._pin_cues_list)
            or station >= len(self._story_clocks)
        ):
            return 0.0
        cues = self._pin_cues_list[station]
        clock = self._story_clocks[station]
        t = clock.get_current_sec()
        if cues.is_on(local_pin, t):
            return value
        return 0.0
