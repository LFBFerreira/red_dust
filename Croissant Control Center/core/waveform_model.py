...
class WaveformModel:
    ...
    def __init__(self, stream: Optional[Stream] = None):
        ...
        self._trace_by_channel: Dict[str, object] = {}
        self._stream_time_range: Optional[Tuple[UTCDateTime, UTCDateTime]] = None
    ...
    def _index_stream(self) -> None:
        """Cache first trace per channel and the overall time span."""
        self._trace_by_channel = {}
        self._stream_time_range = None
        if self._stream is None or len(self._stream) == 0:
            return
        start = None
        end = None
        for trace in self._stream:
            cid = f"{trace.stats.location}.{trace.stats.channel}"
            if cid not in self._trace_by_channel:
                self._trace_by_channel[cid] = trace
        ...
        if start is not None and end is not None:
            self._stream_time_range = (start, end)
    ...
    def _get_trace_for_channel(self, channel_id: Optional[str]):
        if not channel_id:
            return None
        return self._trace_by_channel.get(channel_id)
    ...
    def get_time_range(self) -> Optional[Tuple[UTCDateTime, UTCDateTime]]:
        if self._stream is None or not self._selected_channels:
            return None
        return self._stream_time_range
